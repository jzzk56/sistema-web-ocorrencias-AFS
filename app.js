/* ============================================================
   AFS · Sistema de Ocorrências — app.js
   Vanilla JS SPA: routing, API client, all page renderers
   ============================================================ */
...
function loadingCard() {
  return `<div class="stat-card"><div class="stat-label" style="background:var(--surface-2);height:12px;border-radius:4px;width:60%"></div>
  <div class="stat-value" style="background:var(--surface-2);height:32px;border-radius:6px;width:50%;margin-top:8px"></div></div>`;
}
...
function renderCursoChart(por_curso) {
  ...
  body.innerHTML = `<div class="bar-chart">${por_curso.map(r => `
    <div class="bar-item">
      ...
      <div class="bar-track">
        <div class="bar-fill" style="width:${(r.total/max)*100}%"></div>
      </div>
    </div>`).join('')}</div>`;
}
  ...
  try {
    ...
    const table = `
      <table>
        <thead>
          <tr>
            <th>#</th><th>Aluno</th><th>Turma</th>
            <th>Data</th><th>Gravidade</th><th>Descrição</th><th></th>
          </tr>
        </thead>
        <tbody>
          ${rows.map(r => `
...
function activeFilters() {
  return Object.fromEntries(Object.entries(S.filters).filter(([,v]) => v));
}
...
async function submitForm(id) {
  ...
  ['Nome','Curso','Ano','Data','Gravidade','Desc'].forEach(k => { const e = $('err'+k); if(e) e.textContent=''; });
  ['fNome','fCursoForm','fAnoForm','fData','fDesc'].forEach(id => { const e = $(id); if(e) e.classList.remove('error'); });
...
async function ensureMeta() {
  if (!S.meta.cursos.length) {
    try { S.meta = await API.getMeta(); } catch { /* use defaults */ }
  }
  return S.meta;
}
